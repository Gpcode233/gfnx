...
class LinearGaussianScore(BaseDAGLikelihood):
    ...
    def _local_score(
        ...
        Y = self.prior_scale * jnp.matmul(diffs[:, None], masked_data)
        Y = jnp.squeeze(Y, axis=1)
        Sigma = (self.obs_scale**2) * jnp.eye(num_variables) + (
            self.prior_scale**2
        ) * jnp.matmul(masked_data.transpose(0, 2, 1), masked_data)
        term1 = jnp.sum(diffs**2, axis=1)
        term2 = -jnp.sum(
            Y * jnp.linalg.solve(Sigma, Y[..., None])[..., 0], axis=1
        )
...
class BGeScore(BaseDAGLikelihood):
    ...
    def _local_score(
        ...
        log_term_r = 0.5 * factor * _logdet(self.R, parents) - 0.5 * (
            factor + 1
        ) * _logdet(self.R, parents_and_variable)
