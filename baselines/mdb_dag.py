...
class GNNPolicy(eqx.Module):
    ...
    def __call__(self, adjacency: chex.Array) -> chex.Array:
        B, N, N = adjacency.shape  # Batch, Nodes
        ...
        node_features = node_features[: B * N].reshape(
            B, N, -1
        ...
        node_features = node_features.reshape(B * N, -1)  # [B * N, emb_size]
        ...
        senders = senders.reshape(B, N, -1)  # [B, N, emb_size]
        receivers = jax.vmap(self.receivers_mlp, in_axes=(0))(
            node_features
        )  # [B*N, emb_size]
        receivers = receivers.reshape(B, N, -1)  # [B, N, emb_size]
        ...
        bwd_logits = jnp.zeros((B, self.n_bwd_actions))
    ...
    # TODO: try to optimiese the edge padding to the nearest power of 2, not n^2
    # def _nearest_power_of_2(self, x: chex.Array) -> chex.Array:
    #     """JAX-compatible function to find the next power of 2."""
    #     # Handle x == 0 case
    #     return jax.lax.cond(
    #         x == 0,
    #         lambda _: jnp.array(1, dtype=jnp.int32),
    #         # For x > 0, compute 2^ceiling(log2(x))
    #         lambda _: jnp.power(
    #             2, jnp.ceil(jnp.log2(jnp.maximum(x, 1)))
    #         ).astype(jnp.int32),
    #         operand=None,
    #     )
...
class TrainState(NamedTuple):
    ...
    metrics_module: gfnx.metrics.DAGMetricsModule
    metrics_state: gfnx.metrics.DAGMetricsState
...
def train_step(idx: int, train_state: TrainState) -> TrainState:
    ...
    def logging_callback(idx: int, train_info: dict):
        if (
            idx % train_state.config.logging.track_each == 0
...
def run_experiment(cfg: OmegaConf) -> None:
    ...
    # Load the samples
    samples = gfnx.utils.load_dag_samples(cfg.environment.samples_path)

    # Define the reward function for the environment
    prior = hydra.utils.instantiate(
        cfg.environment.prior,
        num_variables=samples["num_variables"],
    )
    likelihood = hydra.utils.instantiate(
        cfg.environment.likelihood,
    ...
    model = GNNPolicy(
        num_nodes=samples["num_variables"],
    ...
    metrics_module = {
        "corr": gfnx.metrics.DAGMetricsModule(
            # TODO: add DAGMetricsModule
        ),
    }
